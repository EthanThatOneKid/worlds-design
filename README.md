...
│       ├── oxigraph/     # In-Memory RDF Store (Wasm)
...
- **Architecture:**
  ...
  - **Data Plane DBs (`world_*.db`):** Each "World" is an isolated SQLite file
    containing its own `kb_statements`, `kb_chunks`, `kb_limits`, and `kb_usage`
    tables.
- **Value Proposition:**
...
#### World Settings (`/worlds/{world_id}/settings`)
...
- `GET /v1/usage/:account` - Get usage buckets for an account.
...
- `GET /v1/worlds/:world` - Get a specific World graph.
